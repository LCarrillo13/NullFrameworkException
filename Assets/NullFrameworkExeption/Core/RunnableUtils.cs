...
namespace NullFrameworkException
{
    public static class RunnableUtils
    {
        ...
        public static bool Validate<TRunnable>(ref Optional<TRunnable> _runnable, GameObject _from) 
            where TRunnable : IRunnable
        {
            ...
            if(_runnable.IsEmpty())
            {
                _runnable.Update(_from.GetComponent<TRunnable>());
            ...
            if(_runnable.IsEmpty())
            {
                _runnable.Update(_from.GetComponentInChildren<TRunnable>());
        ...
        public static bool Setup<TRunnable>(ref Optional<TRunnable> _runnable, GameObject _from, params object[] _params)
            where TRunnable : IRunnable
        {
            // Validate the component, if we can, set it up and return true
            if(Validate(ref _runnable, _from))
            {
                _runnable.Get().Setup(_params);
                return true;
            }
        ...
        public static void Run<TRunnable>(ref Optional<TRunnable> _runnable, GameObject _from, params object[] _params) 
            where TRunnable : IRunnable
        {
            // Validate the component in case we didn't do it earlier
            if(Validate(ref _runnable, _from))
            {
                _runnable.Get().Run(_params);
            }
        }
    }
}